...
$(document).ready(function () {
    ...
    $('.input').focus(function () {
        ...
        $(this).parent().find(".line").addClass('line-active');
    });

    $(".input").focusout(function () {
        if ($(this).val() == '') {
            $(this).parent().find(".label-txt").removeClass('label-active');
            $(this).parent().find(".line").removeClass('line-active');
        };
    });
...
audio_id.ontimeupdate = function () {
    ...
    var measuredTime = new Date(null);
    measuredTime.setSeconds(audio_currentTime);
    var MHSTime = measuredTime.toISOString().substr(11, 8);

    var spilited_time = MHSTime.split(':'); 
    ...
    if (audio_id.volume == 0) {
        document.getElementById("cs_audio_sound").style.color = "#00179c";
    } else {
